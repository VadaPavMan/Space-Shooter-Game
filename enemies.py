...
class Enemies:
    ...
    def __init__(self, width, height):
            ...
            if self.choose == 1:
                ...
                if self.choose == 1:
                    ...
                    self.enemy = arcade.Sprite(
                        self.monster_high_damage_texture, self.scale
                    )
                else:
                    self.scale = 1.4
                    self.enemy = arcade.Sprite(
                        self.monster_speedshooter_texture, self.scale
                    )
        ...
        if (
            self.enemy.texture == self.big_monster_texture
            or self.enemy.texture == self.monster_high_damage_texture
        ):
            self.max_health = 6
        elif self.enemy.texture == self.monster_texture:
            self.max_health = 5
        elif (
            self.enemy.texture == self.crab_texture
            or self.enemy.texture == self.monster_speedshooter_texture
        ):
            self.max_health = 3
    ...
    def update(self, delta_time, pos_x, pos_y, width, height, other_enemies):
        ...
        for other in other_enemies:
            ...
            if dist_to_other < self.separation_distance and dist_to_other > 0:
                separation_strength = (
                    self.separation_distance - dist_to_other
        ...
        if (
            self.enemy.texture == self.big_monster_texture
            or self.enemy.texture == self.monster_texture
            or self.enemy.texture == self.monster_speedshooter_texture
            or self.enemy.texture == self.monster_high_damage_texture
        ):
            if self.shoot():
                ...
                if self.enemy.texture == self.big_monster_texture:
                    ...
                    new_bullet = shoot.Enemy_Bullet_High_Damage(
                        angle, bullet_x, bullet_y
                    )
                    self.bullets.append(new_bullet)
    ...
    def draw_health_bar(self):
        ...
        arcade.draw_lrbt_rectangle_filled(
            left, green_right, bottom, top, arcade.color.GREEN
        )
